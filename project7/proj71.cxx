...
// YOUR CODE TO GENERATE ISOLINES SHOULD GO HERE!
	...
	pidx[0][0] = ptr[0];
	pidx[0][1] = ptr[1];
	v[0] = F[GetPointIndex(pidx[0], dims)];
	pidx[1][0] = ptr[0]+1;
	pidx[1][1] = ptr[1];
	v[1] = F[GetPointIndex(pidx[1], dims)];
	pidx[2][0] = ptr[0];
	pidx[2][1] = ptr[1]+1;
	v[2] = F[GetPointIndex(pidx[2], dims)];
	pidx[3][0] = ptr[0]+1;
	pidx[3][1] = ptr[1]+1;
	v[3] = F[GetPointIndex(pidx[3], dims)];
	setEdgeIndex(E, pidx, dims);
	int icase = 0;
	int x;
	for(x = 0; x < 4; x++){
	   if(v[x] > 3.2){
	...
	float pt1[2]; //interpolate
	float pt2[2]; //interpolate
	int nsegments= numsegments[icase];
	for(x = 0; x < nsegments; x++){
	    int edge1 = table[icase][2*x];
	    getEdges(ptscalar, edge1);
	    t = (3.2 - F[E[edge1][0]]) / (F[E[edge1][1]] - F[E[edge1][0]]);
	    
	    pt1[0] = X[pidx[ptscalar[0]][0]] + (t *(X[pidx[ptscalar[1]][0]] - X[pidx[ptscalar[0]][0]]));
	    pt1[1] = Y[pidx[ptscalar[0]][1]] + (t *(Y[pidx[ptscalar[1]][1]] - Y[pidx[ptscalar[0]][1]]));
	    int edge2 = table[icase][2*x+1];
	    getEdges(ptscalar, edge2);
	    t = (3.2 - F[E[edge2][0]]) / (F[E[edge2][1]] - F[E[edge2][0]]);
	    pt2[0] = X[pidx[ptscalar[0]][0]] + (t *(X[pidx[ptscalar[1]][0]] - X[pidx[ptscalar[0]][0]]));
	    pt2[1] = Y[pidx[ptscalar[0]][1]] + (t *(Y[pidx[ptscalar[1]][1]] - Y[pidx[ptscalar[0]][1]]));
